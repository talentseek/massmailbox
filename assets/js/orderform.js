        ...
        domainDiv.innerHTML = `
            <div class='form-group mb-2'>
                ...
                <div class="form-check mt-2">
                    ...
                    <label class="form-check-label" for='registerDomain_${domainIndex}'>Register New Domain ($12.00)</label>
                </div>
            </div>
...
function updateOrderSummary() {
    ...
    let formattedSummary = '<ul class="list-group">';
    ...
    domains.forEach(domain => {
        ...
        formattedSummary += `<li class="list-group-item">Domain: ${domain.name}, Forwarding: ${domain.forwarding}, Register: ${domain.register ? 'Yes' : 'No'}, Mailboxes: ${domain.mailboxes.join(', ')}</li>`;
    });

    totalMailboxCost = calculateMailboxCost(totalMailboxes);

    formattedSummary += '</ul>';
    formattedSummary += `<div class="card mt-3"><div class="card-body">`;
    formattedSummary += `<h5 class="card-title">Order Summary</h5>`;
    formattedSummary += `<p class="card-text">Total Domain Registration Fees: <strong>$${totalDomainCost.toFixed(2)}</strong></p>`;
    formattedSummary += `<p class="card-text">Total Mailbox Fees: <strong>$${totalMailboxCost.toFixed(2)} per month</strong></p>`;
    // formattedSummary += `<p class="card-text">Total Number of Mailboxes: ${totalMailboxes}</p>`;
    formattedSummary += `</div></div>`;

    const orderSummaryDiv = document.getElementById('orderSummary');
    orderSummaryDiv.innerHTML = formattedSummary;
...
document.getElementById('orderForm').addEventListener('submit', function(event) {
    ...
    emailjs.sendForm('service_yzmsej5', 'template_yvgx87b', this)
        .then(function() {
            console.log('SUCCESS!');
            // Redirect to the thank you page instead of showing an alert
